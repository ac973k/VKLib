...
namespace VK {
    namespace Utils {
        ...
        {
            QUrl link(QString("https://api.vk.com/method/utils.deleteFromLastShortened?access_token=%1&key=%2&v=5.131").arg(access_token).arg(key));
            qDebug() << "full url: " << link;
            QNetworkRequest request(link);
            QNetworkReply* reply = networkManager.get(request);
            connect(reply, &QNetworkReply::finished, this, &Utils::Finished);
        ...
        {
            QUrl link(QString("https://api.vk.com/method/utils.getLastShortenedLinks?access_token=%1&count=%2&v=5.131").arg(access_token).arg(count));
            qDebug() << "full url: " << link;
            QNetworkRequest request(link);
            QNetworkReply* reply = networkManager.get(request);
            connect(reply, &QNetworkReply::finished, this, &Utils::Finished);
        ...
        {
            QUrl link(QString("https://api.vk.com/method/utils.getLinkStats?access_token=%1&key=%2&interval=%3&intervals_count=%4&extended=%5&v=5.131")
                          .arg(access_token)
                          .arg(key)
                          .arg(interval)
                          .arg(intervals_count)
        ...
        {
            QUrl link(QString("https://api.vk.com/method/utils.getServerTime?access_token=%1&v=5.131").arg(access_token));
            qDebug() << "full url: " << link;
            QNetworkRequest request(link);
            QNetworkReply* reply = networkManager.get(request);
            connect(reply, &QNetworkReply::finished, this, &Utils::Finished);
        ...
        {
            QUrl link(QString("https://api.vk.com/method/utils.getShortLink?access_token=%1&url=%2&priv=%2&v=5.131").arg(access_token).arg(url).arg(priv));
            qDebug() << "full url: " << link;
            QNetworkRequest request(link);
            QNetworkReply* reply = networkManager.get(request);
            connect(reply, &QNetworkReply::finished, this, &Utils::Finished);
        ...
        {
            QUrl link(QString("https://api.vk.com/method/utils.resolveScreenName?access_token=%1&screen_name=%2&v=5.131").arg(access_token).arg(screen_name));
            qDebug() << "full url: " << link;
            QNetworkRequest request(link);
            QNetworkReply* reply = networkManager.get(request);
            connect(reply, &QNetworkReply::finished, this, &Utils::Finished);
